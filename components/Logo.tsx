const LOGO_WIDTH = 826.4
const LOGO_HEIGHT = 132.4
...
export function getLogoSvgMarkup(fill = '#0D2340', height = 26) {
  const width = Number((height * LOGO_ASPECT_RATIO).toFixed(1))
  return `<svg viewBox="0 0 ${LOGO_WIDTH} ${LOGO_HEIGHT}" xmlns="http://www.w3.org/2000/svg" style="height:${height}px;width:${width}px;display:block;overflow:visible;fill:${fill}">${LOGO_PATHS}</svg>`
}

export function Logo({ fill = '#0D2340', height = 26, className }: Props) {
  const width = Number((height * LOGO_ASPECT_RATIO).toFixed(1))
  ...
  return (
    <svg
      ...
      width={width}
      height={height}
      style={{ display: 'block', overflow: 'visible', fill }}
